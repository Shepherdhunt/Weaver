...
| `buildcheck.py` | tracker §7 | Does validation build what was analysed? Recording shims first on `PATH` (one file per compile, so parallel builds never interleave) during the baseline build. Flags that change what C code means (defines, dialect, include paths inside the project or the SDK, target, language `-f` flags) are compared with the analysed commands per source file. Also compared: the compiler, and sources on one side only (ignoring generated sources and build-system probes). Result: the `configuration` record; the last result per profile is kept for `weaver doctor`, and `doctor --build` runs it on demand. |
| `Dockerfile`, `container/weaver-docker` | — | The pinned toolchain image (default, `svf` and `test` targets) and the wrapper that runs it on a project: same-path mount, host user, loopback-only publishing. |
| `doctor.py` | — | `weaver doctor`: machine checks made by running the tools (a JSON AST from Clang, an LTO link with GCC, a `--coverage` link with each compiler), SVF, binutils, build tools, memory, disk and clock; project checks against `weaver.yaml` and the analysis; install hints per package manager. `identify` is called without its cache, so a tool installed while the server runs shows up. |
