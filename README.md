...
| **Recipe `output-param`** (tracker roadmap) | Turns a pointer parameter the function only writes into a return value. A `void` function returns the value (`get(a, &v);` → `v = get(a);`); a function that returns a status returns a small result record declared next to its prototype (`s = read(&v);` → the record's `status` and `value`). 10 preconditions, including: the target is written on every path before every return (a definitely-assigned analysis, with null tests of the parameter folded because every caller passes `&variable`), and every caller passes the address of a whole local variable whose address is taken nowhere else, so nothing else can reach it during the call. A caller that never reads the variable drops the value and the variable. |
